...
const recordLabels: Record<string,string> = { service:'Revizie',maintenance:'Mentenanță',oil_change:'Schimb ulei',filters:'Filtre',brakes:'Frâne',tires:'Anvelope',repair:'Reparație',inspection:'Inspecție',itp:'ITP',rca:'RCA',casco:'CASCO',fire_extinguisher:'Stingător',fuel:'Combustibil',tax:'Taxe',vignette:'Rovinietă',parking:'Parcare',mileage:'Kilometraj',purchase:'Achiziție' }
...
function monthLabel(value: string) { const parsed=new Date(`${value}-01T12:00:00Z`); return Number.isNaN(parsed.getTime()) ? value : parsed.toLocaleDateString('ro-RO',{month:'long',year:'numeric',timeZone:'UTC'}) }
...
function VehicleGallery({ vehicle, compact = false }: { vehicle: Vehicle; compact?: boolean }) {
  ...
  const url = photo && !failed[key] ? photo.data_url || urls[key] || null : null
...
function VehicleDetail({vehicle,records,documents,documentsLoading,demo,today,onBack,onOpenDocument}: {vehicle: Vehicle;records: VehicleRecord[];documents: DocumentRow[];documentsLoading:boolean;demo:boolean;today:string;onBack:()=>void;onOpenDocument:(id:string)=>void}) {
  ...
  const technical = [
    ...
    ['Locuri',text(vehicle.extra?.seats)],['Masă maximă autorizată',text(vehicle.extra?.max_authorized_mass_kg) !== null ? `${Number(text(vehicle.extra?.max_authorized_mass_kg)).toLocaleString('ro-RO')} kg` : null],
  ...
  const deadlineItems = [
    ...
    {key:'fire_extinguisher',label:'Stingător',types:['fire_extinguisher']},
    ...
    const remainingMileage=spec.key==='service'&&record ? text(record.extra?.remaining_km) : null
    const nextDueMonth=spec.key==='service'&&record ? text(record.extra?.next_due_month) : null
    return {...spec,record,due,nextMileage,remainingMileage,nextDueMonth,status:(remainingMileage||nextDueMonth)&&!due ? {label:'Planificat',tone:'ok'} : deadlineStatus(due,today)}
  })
  const events = [
  ...
  const known = new Set(['vehicle_type','mileage_km','fuel','power_kw','transmission','color','seats','max_authorized_mass_kg','inspection_due','purchase_date','photos'])
