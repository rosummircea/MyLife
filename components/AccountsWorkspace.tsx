...
function AccountCard({account,ownerName}:{account:Account;ownerName:string}) {
  ...
  return <article className="accountsBalanceCard"><header><div className="accountsBankIcon">{bank.icon?<Image src={bank.icon} alt={bank.name} width={32} height={32}/>:account.account_type==='cash'?<Banknote size={24}/>:<Landmark size={24}/>}</div><div><h4 title={account.name}>{title}</h4>{credit && available!==null && <span className="accountsAvailableLabel">Disponibil pe card</span>}<span>{bank.name} · {accountTypes[account.account_type] || account.account_type.replace(/_/g,' ')}</span></div></header><div className="accountsAmountArea"><span>{credit && available!==null?'Disponibil pe card':credit?'Sold card de credit':'Sold'}</span><strong className={`accountsAmount accountsAmount-${color}`}>{money(available ?? balance,currency)}</strong></div>{credit && <div className="accountsCreditDetails">{balance<0?<p><span>Datorie</span><strong className="accountsAmount-red">−{money(Math.abs(balance),currency)}</strong></p>:<p><span>Sold card</span><strong>{money(balance,currency)}</strong></p>}{available===null && <small>Limita nu este disponibilă; disponibilul nu poate fi calculat.</small>}</div>}</article>
}
